...
from keep_alive import keep_alive  # ✅ Render フラスク起動
        ...
        if not role:
            try:
                ...
                await interaction.response.send_message(
                    f"ロール {role_name} を作成できません（権限不足）", ephemeral=True
                )
                return
...
# ✅ リマインド（あなたのコードそのまま）
# ---------------------------------------------------------
# ✅（あなたのリマインド復元・モーダル・削除機能のコードをここにそのまま配置）
