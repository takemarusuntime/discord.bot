...
intents.reactions = True  # リアクション付与/削除イベント必須
...
@bot.tree.command(name="b2_リマインド設定", description="指定した時間にリマインドします")
...
@bot.tree.command(
    name="x1_リアクションロール設定",
    description="リアクションロールを新規作成（本文はモーダルで入力）"
)
...
async def x1_rr_setup(interaction: discord.Interaction, 絵文字とロール: str, 複数選択: bool = True):
    ...
    class RRMessageModal(discord.ui.Modal, title="本文メッセージ入力"):
        ...
        async def on_submit(self, mi: discord.Interaction):
            ...
            save_reaction_roles()
            await mi.response.send_message(
                f"作成完了: メッセージID {msg.id} / 排他: {'ON' if not 複数選択 else 'OFF'}",
                ephemeral=True
            )

    await interaction.response.send_modal(RRMessageModal())

@bot.tree.command(name="y1_リアクションロール追加", description="既存のリアクションロールにペアを追加")
...
@bot.tree.command(name="y2_リアクションロール削除", description="既存のリアクションロールから絵文字を削除")
...
@bot.tree.command(name="y3_リアクションロール本文編集", description="対象メッセージの本文を編集")
...
# ============================== 問い合わせ /x2_問い合わせ設定 ==============================
@bot.tree.command(name="x2_問い合わせ設定", description="問い合わせボタンを設置（本文はモーダルで入力）")
...
@bot.tree.command(name="x3_ピン留め設定", description="このチャンネルに案内メッセージを固定（本文はモーダル）")
@app_commands.default_permissions(administrator=True)
...
@bot.tree.command(name="x4_ピン留め削除", description="このチャンネルのピン留めを削除")
@app_commands.default_permissions(administrator=True)
